...
from .choices import price_choices, bedroom_choices, state_choices  # no need listings, already imported Listing
...
def listing(request, listing_id):
    listing = get_object_or_404(Listing,
                                pk=listing_id)  # if page doesn't exist, it will show page not found(error message)
    ...
    if 'bedrooms' in request.GET:
        ...
        if bedrooms:
            queryset_list = queryset_list.filter(
                bedrooms__lte=bedrooms)  # Suppose, 4 bedrooms(Upto). lte means less then or equalto
